...
class Nwdthemes_Revslider_Helper_Framework extends Mage_Core_Helper_Abstract {
	...
	public function __construct() {
        ...
        self::$WP_PLUGIN_URL = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . Nwdthemes_Revslider_Helper_Plugin::WP_PLUGIN_DIR;

        self::$RS_PLUGIN_PATH = Mage::getSingleton('core/design_package')->getSkinBaseDir(array('_theme' => 'default')) . '/nwdthemes/revslider/';
        self::$RS_PLUGIN_SLUG = $this->apply_filters('set_revslider_slug', Nwdthemes_Revslider_Helper_Data::REVSLIDER_PRODUCT);
        ...
        $this->_imagesHelper = Mage::helper('nwdrevslider/images');
    ...
    public function wp_get_current_user() {
        ...
        $user = (object) array(
            'display_name' => $admin->getUsername()
        );
        return $user;
    }
    ...
    public function get_option($handle, $default = false) {
        if ( $value = Mage::getModel('nwdrevslider/options')->getOption($handle, $default) ) {
            if ((strpos($value, 'a:') !== false
                ...
                $this->update_option($handle, $unserializedValue);
                $value = $unserializedValue;
            }
            return $value;
        } else {
    ...
    /**
     * Replace matches callback
     *
     * @param array $matches
     * @return string
     */
    protected function _replaceMatches($matches) {
    ...
    public function plugins_url($file, $plugin) {
        return Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . Nwdthemes_Revslider_Helper_Plugin::WP_PLUGIN_DIR . $this->_pluginHelper->getPluginName($plugin) . '/' . $file;
    }
    ...
    public function plugin_dir_path($plugin) {
        return Nwdthemes_Revslider_Helper_Plugin::getPluginDir() . $this->_pluginHelper->getPluginName($plugin) . '/';
	}
	...
	public function wp_get_attachment_image_src($attachment_id, $size='thumbnail') {
		return Mage::helper('nwdrevslider/images')->wp_get_attachment_image_src($attachment_id, $size);
	}
	 ...
	 * Get images helper
	 *
	 * @return Nwdthemes_Revslider_Helper_Images
	 */
	...
	protected function _makeImagesRelative($data) {
		if (is_array($data)) {
			...
			$data = Nwdthemes_Revslider_Helper_Images::MEDIA_URL_PACEHOLDER . $this->getImagesHelper()->imageFile($data);
		}
		return $data;
	}
	...
	protected function _makeImagesAbsolute($data) {
		if (is_array($data)) {
			foreach ($data as $key => $item) {
				...
				// fix missing image url here
				if ($key == 'media' && is_array($item) && ! empty($item['imageId']) && empty($item['imageUrl'])) {
					$item['imageUrl'] = $this->getImagesHelper()->imageFile($this->getImagesHelper()->get_attached_file($item['imageId']));
				}

				$data[$key] = $this->_makeImagesAbsolute($item);
			}
		} elseif (is_string($data) && strpos($data, Nwdthemes_Revslider_Helper_Images::MEDIA_URL_PACEHOLDER) === 0) {
			if ($this->getImagesHelper()->isMedia($data)) {
				$data = $this->getImagesHelper()->imageUrl(str_replace(Nwdthemes_Revslider_Helper_Images::MEDIA_URL_PACEHOLDER, '', $data));
			} else {
				$data = str_replace(Nwdthemes_Revslider_Helper_Images::MEDIA_URL_PACEHOLDER, Mage::getBaseUrl(), $data);
			}
		}
		return $data;
	}
