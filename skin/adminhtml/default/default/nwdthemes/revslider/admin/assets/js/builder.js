	...
	*/
	function initHoversAndSelects() {
		...
		RVS.DOC.on('click','#rb_editor_logo',function() {
            ...
            jQuery('.nav-bar').addClass('show_magento_menu');
			punchgs.TweenLite.to('.nav-bar', 0.5,{opacity:1});
		});

		RVS.DOC.on('mouseenter','.nav-bar, #wpadminbar',function() {
			clearTimeout(hideWPMENUTIMER);
			jQuery('.rs-builder-mode.hideallwp').addClass("showwpmenus");
			punchgs.TweenLite.to('.nav-bar', 0.5,{opacity:1});
		});

		RVS.DOC.on('mouseleave','.nav-bar, #wpadminbar',function() {
			hideWPMENUTIMER = setTimeout(function() {
				punchgs.TweenLite.to('.nav-bar, #wpadminbar', 0.1,{opacity:0,onComplete:function() {
                    jQuery('.rs-builder-mode.hideallwp').removeClass("showwpmenus");
                    jQuery('.nav-bar').removeClass('show_magento_menu');
				}});
			},200);
		});
