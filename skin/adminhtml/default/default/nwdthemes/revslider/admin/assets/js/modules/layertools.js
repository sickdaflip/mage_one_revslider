...
(function() {
	...
	*/
	...
	function resizeLayer(obj) {
		...
		if (jQuery.inArray(l.type,["text","button"])>=0) for (var a in RVS.RSCBA.t) RVS.F.updateSliderObj({path:p+'.idle.'+RVS.RSCBA.t[a]+'.'+RVS.screen+".v",val:reLaMu("round",_[RVS.RSCBA.t[a]],obj.size),ignoreResponsive:obj.ignoreResponsive, ignoreBackup:obj.ignoreBackup});
		for (var a in RVS.RSCBA.a) RVS.F.updateSliderObj({path:p+'.size.'+RVS.RSCBA.a[a]+'.'+RVS.screen+".v",val:reLaMu("round",_[RVS.RSCBA.a[a]],obj.size),ignoreResponsive:obj.ignoreResponsive, ignoreBackup:obj.ignoreBackup});
