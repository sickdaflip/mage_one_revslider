...
class RevSliderSlider extends RevSliderFunctions {
	...
	/**
	 * old version of Mage::helper('nwdrevslider/framework')->get_param();
	...
	public function init_by_id($sid){
		...
		$slider_data = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE id = %d", $sid), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
		if(empty($slider_data) && !Mage::helper('nwdrevslider/framework')->is_admin()){
	...
	public function init_by_alias($alias){
		...
		$slider_data = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE alias = %s", $alias), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
		if(empty($slider_data)){
			$alias = str_replace('-', ' ', $alias); //go back to an very old option where an slider alias could have a space
			$slider_data = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE alias = %s", $alias), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
		}
		if(empty($slider_data) && !Mage::helper('nwdrevslider/framework')->is_admin()){
			throw new Exception('Slider with alias '.Mage::helper('nwdrevslider/framework')->sanitize_text_field(esc_attr($alias)).' not found.');
		}
	...
	public function init_by_data($data){
		$data = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slider_init_by_data', $data);
		...
		Mage::helper('nwdrevslider/framework')->do_action('revslider_slider_init_by_data_post', $this);
	}
	...
	public function get_shortcode(){
		return '{{block type="nwdrevslider/revslider" alias="'.$this->alias.'"}}';
	}
	...
	public function get_slider_count(){
		...
		$count = count($wpdb->get_results("SELECT COUNT(*) FROM ".$wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE `type` = '' OR `type` IS NULL", Nwdthemes_Revslider_Helper_Query::ARRAY_A));
	...
	public function get_first_slide_id_from_gallery(){
		...
		$slides = array();
		$record = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %s ORDER BY slide_order ASC LIMIT 0,1", array($this->get_id())), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	public function get_alias_by_id($slider_id){
		...
		$record = $wpdb->get_row($wpdb->prepare("SELECT `alias` FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE id = %s LIMIT 0,1", array($slider_id)), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
				...
				if($image != ''){
					...
					switch($altOption){
						case 'media_library';
							...
							if($id > 0) $alt = Mage::helper('nwdrevslider/framework')->get_post_meta($id, '_wp_attachment_image_alt', true);
						break;
						case 'file_name';
							$alt = $image;
						break;
					...
					switch($titleOption){
						case 'media_library';
							$id = attachment_url_to_postid($image);
							if($id > 0) $title = Mage::helper('nwdrevslider/framework')->get_the_title($id);
						break;
						case 'file_name';
							$title = $image;
						break;
	...
	public static function alias_exists($alias, $return_id = false){
		...
		$alias_exists = $wpdb->get_row($wpdb->prepare("SELECT id FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE alias = %s", $alias), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	private function delete_all_slides(){
		...
		Mage::helper('nwdrevslider/framework')->do_action('revslider_slider_delete_all_slides', $this->id);
	...
	public function delete_static_slide(){
		...
		Mage::helper('nwdrevslider/framework')->do_action('revslider_slider_delete_static_slide', $this->id);
	}
	...
	public function duplicate_slider_by_id($id, $is_template = false){
		...
		if($is_template){
			...
			$talias	= strtolower(Mage::helper('nwdrevslider/framework')->sanitize_title_with_dashes($title));
		}else{
		...
		while($this->alias_exists($talias)){ //set a new alias and title if its existing in database
			$talias = strtolower(Mage::helper('nwdrevslider/framework')->sanitize_title_with_dashes($title)) .  '-' . $ti;
			$ti++;
		}
	...
	public function update_title($new_title){
		...
		$new_title = stripslashes(Mage::helper('nwdrevslider/framework')->esc_html($new_title));
		if(!empty($new_title)){
	...
	public function update_slider_tags($slider_id, $tags){
		...
		$record	  = $wpdb->get_row($wpdb->prepare("SELECT `settings` FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE id = %s", $slider_id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
		$cur_tags = array();
	...
	public function get_last_slider_id(){
		...
		$record = $wpdb->get_row("SELECT `id` FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." ORDER BY `id` DESC LIMIT 0,1", Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	private function duplicate_slider($title = false, $prefix = false){
		...
		if($title === false){
			...
			$new_title = ($prefix !== false) ? Mage::helper('nwdrevslider/framework')->sanitize_text_field($title.' '.$this->get_val($params, 'title')) : Mage::helper('nwdrevslider/framework')->sanitize_text_field($title);
			$new_alias = ($prefix !== false) ? Mage::helper('nwdrevslider/framework')->sanitize_title($title.' '.$this->get_val($params, 'title')) : Mage::helper('nwdrevslider/framework')->sanitize_title($title);
		...
		$params['shortcode'] = '{{block type="nwdrevslider/revslider" alias="'. $new_alias .'"}}';
		...
		$slides_ids = array();
		$slides = $wpdb->get_results($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %s", $this->id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
		if(!empty($slides)){
		...
		$static_id	= 0;
		if($staticID !== false){
			$record = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES ." WHERE id = %s", $staticID), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
			unset($record['id']);
		...
		//update actions
		$slides = $wpdb->get_results($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %s", $slider_last_id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
		if($static_id > 0){
			$slides_static = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES ." WHERE id = %s", $static_id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	public function check_alias($alias){
		...
		$slider	= $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE alias = %s", $alias).$add, Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	public function create_blank_slider(){
		...
		$slider_data = array(
			...
			'settings'	=> json_encode(array('version' => Nwdthemes_Revslider_Helper_Framework::RS_REVISION), JSON_FORCE_OBJECT),
			'type'		=> ''
		);
	...
	public function save_slider($slider_id, $data){
		...
		$title	= Mage::helper('nwdrevslider/framework')->sanitize_text_field($this->get_val($params, 'title'));
		$alias	= Mage::helper('nwdrevslider/framework')->sanitize_text_field($this->get_val($params, 'alias'));
	...
	public function get_sliders($templates = false){
		...
		$slider_data = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_SLIDER." WHERE `type` != 'folder' ORDER BY %s %s", array($do_order, $direction)), Nwdthemes_Revslider_Helper_Query::ARRAY_A); //WHERE `type` = '' OR `type` IS NULL
		if(!empty($slider_data)){
	...
	public function get_slides_for_export(){
		...
		return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_slides_for_export', Mage::helper('nwdrevslider/framework')->apply_filters('revslider_getSlidesForExport', $export));
	}
	...
	public function get_static_slide_for_export(){
		...
		return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_getStaticSlideForExport', $static_slide);
	}
	...
	public function get_parent_slides($published, $gal_ids, $lang){
		...
		Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_parent_slides_pre', $lang, $published, $gal_ids, $this);
		...
		Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_parent_slides_post', $parent_slides, $published, $gal_ids, $this);
	...
	public function get_max_order(){
		...
		$record = $wpdb->get_row($wpdb->prepare("SELECT slide_order FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %d ORDER BY slide_order DESC LIMIT 0,1", $this->id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	public function copy_slide_to_slider($data){
		...
		$add_to_slider	= $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE id = %s", $slider_id), Nwdthemes_Revslider_Helper_Query::ARRAY_A); //check if ID exists

		if(empty($add_to_slider))
			return __('Slide could not be duplicated', 'revslider');

		//get last slide in slider for the order
		$slide_order	= $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %s ORDER BY slide_order DESC", $slider_id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
		$slide_to_copy	= $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE id = %s", $slide_id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	public function get_slides_from_stream($published = false){
		...
		$posts = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_pre_mod_stream_data', $posts, $sourcetype, $this->id);
		...
		$posts = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_post_mod_stream_data', $posts, $sourcetype, $this->id);
	...
	private function get_posts_by_categories($published = false){
		...
		return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_posts_by_categories', $post, $this);
	}
	...
	public function get_overview_data($slider = false){
		...
		return array(
			...
			'slide_id'	=> $id,
			'slide_ids'	=> $slide_ids,
			'title'		=> Mage::helper('nwdrevslider/framework')->esc_html($slider->get_title()),
			'alias'		=> $slider->get_alias(),
			'source'	=> Mage::helper('nwdrevslider/framework')->esc_html($type),
	...
	public function get_specific_posts($gal_ids = array()){
		...
		if(!empty($gal_ids) && $gal_ids[0] !== ''){
			$posts	= $gal_ids;
			$posts	= Mage::helper('nwdrevslider/framework')->apply_filters('revslider_set_posts_list_gal', $posts, $this->get_id());
			$is_gal	= true;
		}else{
			if(isset($gal_ids[0])){
				unset($gal_ids[0]);
			...
			$posts = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_set_posts_list', $posts, $this->get_id());
		}
	...
	public function get_posts_by_id($ids, $slider_id, $is_gal, $additional = array()){
		...
		$posts= Mage::helper('nwdrevslider/products')->getProductsByQuery($query);
	...
	public function get_posts_by_category($slider_id, $cat_id, $sort_by = 'ID', $direction = 'DESC', $max_posts = -1, $post_types = 'any', $taxonomies = 'category', $addition = array(), $type = ''){
		$a = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_posts_by_category', array('slider_id' => $slider_id, 'cat_id' => $cat_id, 'sort_by' => $sort_by, 'direction' => $direction, 'max_posts' => $max_posts, 'post_types' => $post_types, 'taxonomies' => $taxonomies, 'addition' => $addition, 'type' => $type), $this);
	...
	public function get_post_categories($post_id, $tax){
		...
		$cats = Mage::helper('nwdrevslider/framework')->wp_get_post_terms($post_id, $tax);
