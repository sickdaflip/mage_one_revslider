...
class RevSliderSlide extends RevSliderFunctions {
	...
	public function get_settings(){
		return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_get_settings', Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_getSettings', $this->settings, $this), $this);
	}
	...
	public function get_params(){
		return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_get_params', Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_getParams', $this->params, $this), $this);
	}
	...
	public function get_layers(){
		return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_layers', Mage::helper('nwdrevslider/framework')->apply_filters('revslider_getLayers', $this->layers, $this), $this);
	}
	...
	public function get_static_slide_id($slider_id){
		...
		if(!empty($slider_id)){
			$slide = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES." WHERE slider_id = %d", $slider_id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
		}
	...
	public function exist_by_id($slideid){
		...
		try{
			if(strpos($slideid, 'static_') !== false){
				...
				$record = $wpdb->get_row($wpdb->prepare("SELECT id FROM ". $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES ." WHERE id = %d", $slide_id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
			}else{
				$record = $wpdb->get_row($wpdb->prepare("SELECT id FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE id = %d", $slideid), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
			}
		}catch(Exception $e){
			return false;
		}
	...
	public function init_by_id($slide_id){
		...
		try{
			if(strpos($slide_id, 'static_') !== false){
				...
				$slide = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES ." WHERE id = %d", $static_id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
			}else{
				$this->validate_numeric($slide_id, 'Slide ID');
				$slide = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE id = %d", $slide_id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
			}
	...
	public function save_slide($slide_id, $data, $slider_id){
		...
		$this->settings['version'] = $this->get_val($params, 'version', $this->get_val($this->settings, 'version', Nwdthemes_Revslider_Helper_Framework::RS_REVISION));
		...
		// $update = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_updateSlideFromData_pre', $update, $data, $this);
		...
		Mage::helper('nwdrevslider/framework')->do_action('revslider_slide_updateSlideFromData_post', false, $data, $this);
	...
	public function delete_slide_by_id($slide_id){
		...
		Mage::helper('nwdrevslider/framework')->do_action('revslider_slide_deleteSlide', $slide_id);
	}
	...
	public function duplicate_slide_by_id($slide_id, $slider_id){
		...
		$done	= false;
		$slide	= $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE id = %s", $slide_id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
		if(!empty($slide)){
	...
	public function init_by_post_data($data, RevSliderSlide $template, $slider_id){
		...
		$template_id	 = Mage::helper('nwdrevslider/framework')->get_post_meta($post_id, 'slide_template', true);
		$template_id	 = ($template_id == '') ? 'default' : $template_id;
		$this->post_data = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByPostData', $data, $template, $slider_id, $this);
		...
		if($this->get_val($this->params, array('seo', 'set'), false) == true && $this->get_val($this->params, array('seo', 'type'), 'regular') == 'regular'){
			$link = Mage::helper('nwdrevslider/framework')->get_permalink($post_id);
			...
			foreach($matches as $match){
				...
				$meta_val = Mage::helper('nwdrevslider/framework')->get_post_meta($post_id, $meta, true);
		...
		if($this->get_val($this->params, array('bg', 'imageFromStream'), false) === true){ //if image is choosen, use featured image as background // && $this->get_val($this->params, array('bg', 'type')) == 'image'
			$tid = Mage::helper('nwdrevslider/framework')->get_post_thumbnail_id($post_id);

			if(!empty($tid)){
				$this->set_image_by_image_id($tid);

				//set the thumbnail image
				$thumbnail_url = Mage::helper('nwdrevslider/images')->wp_get_attachment_image_src($tid, 'thumbnail');
	...
	private function set_layers_by_post($post, $slider_id){
		...
		$post = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_setLayersByPostData_pre', $post, $slider_id, $this);
		...
		$ptid		= Mage::helper('nwdrevslider/framework')->get_post_thumbnail_id($post['ID']);
		$attr		= array(
		...
		$attr = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_setLayersByPostData_post', $attr, $post, $slider_id, $this);

		if(!empty($this->layers)){
			foreach($this->layers as $key => $layer){

				$text = $this->get_val($layer, 'text');
				$text = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_mod_meta', $text, $post['ID'], $post); //option to add your own filter here to modify meta to your likings
				...
				if($this->get_val($layer, 'type', 'text') === 'image' && $this->get_val($layer, array('media', 'imageFromStream'), false) === true){
					$featured_image_url = Mage::helper('nwdrevslider/images')->wp_get_attachment_image_src($ptid, 'full');
	...
	public function get_excerpt_by_id($id, $limit = 55){
		...
		$post	 = Mage::helper('nwdrevslider/framework')->get_post($id);
		...
		if(!empty($arrMatches)){
			foreach($arrMatches as $matched){
				foreach($matched as $match) {
					...
					$avatar = Mage::helper('nwdrevslider/framework')->get_avatar_url($this->get_val($attr, 'authorID'), array('size'=> $vals[0]));
	...
	public function init_by_stream_data($data, $template, $slider_id, $sourcetype, $additions){
		...
		$a = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByStreamData', array('post_data' => $data, 'template' => $template, 'slider_id' => $slider_id, 'sourcetype' => $sourcetype, 'additions' => $additions), $this);
	...
	private function init_by_facebook($slider_id, $additions){
		$this->post_data = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByFacebook_pre', $this->post_data, $slider_id, $additions, $this);
		...
		if($this->get_val($this->params, array('bg', 'type')) == 'image'){ //if image is choosen, use featured image as background
			...
			$this->image_url = (empty($this->image_url)) ? Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/facebook.png' : $this->image_url;
			$this->image_url = (Mage::helper('nwdrevslider/framework')->is_ssl()) ? str_replace('http://', 'https://', $this->image_url) : $this->image_url;
			$this->image_filename = basename($this->image_url);
		}

		$this->post_data = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByFacebook_post', $this->post_data, $slider_id, $additions, $this);
	...
	private function init_by_twitter($slider_id, $additions){
		$this->post_data = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByTwitter_pre', $this->post_data, $slider_id, $additions, $this);
		...
		if($bg_type == 'trans' || $bg_type == 'image' || $bg_type == 'streamtwitter' || $bg_type == 'streamtwitterboth'){ //if image is choosen, use featured image as background
			...
			if($img == ''){
				...
				if(is_array($image_url_array) && isset($image_url_array[0])){
					$img = (Mage::helper('nwdrevslider/framework')->is_ssl()) ? $this->get_val($image_url_array[0], 'media_url_https') : $this->get_val($image_url_array[0], 'media_url');
				}
			}
			...
			$this->image_url = (empty($this->image_url)) ? Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/twitter.png' : $this->image_url;
			$this->image_url = (Mage::helper('nwdrevslider/framework')->is_ssl()) ? str_replace("http://", "https://", $this->image_url) : $this->image_url;
			$this->image_filename = basename($this->image_url);
		}

		$this->post_data = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByTwitter_post', $this->post_data, $slider_id, $additions, $this);
	...
	private function init_by_instagram($slider_id, $additions = array()){
		$this->post_data = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByInstagram_pre', $this->post_data, $slider_id, $this);
		...
		if(in_array($this->get_val($this->params, array('bg', 'type')), array('trans', 'image', 'streaminstagram', 'streaminstagramboth'), true)){ //if image is choosen, use featured image as background
			...
			*/
			...
			$this->image_url = (empty($this->image_url)) ? Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/instagram.png' : $this->image_url;
			$this->image_url = (Mage::helper('nwdrevslider/framework')->is_ssl()) ? str_replace('http://', 'https://', $this->image_url) : $this->image_url;
			$this->image_filename = basename($this->image_url);
		}
		...
		$this->post_data = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByInstagram_post', $this->post_data, $slider_id, $this);
	...
	private function init_by_flickr($slider_id, $additions){
		$this->post_data = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByFlickr_pre', $this->post_data, $slider_id, $this);
		$this->id		 = $this->get_val($this->post_data, 'id');
		$this->set_param('title', $this->get_val($this->post_data, 'title'));
		$this->set_param(array('publish', 'state'), 'published');
		...
		if($this->get_val($this->params, array('bg', 'type')) == 'image'){ //if image is choosen, use featured image as background
			...
			$this->image_url	= (empty($this->image_url)) ? Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/flickr.png' : $this->image_url;
			$this->image_url	= (Mage::helper('nwdrevslider/framework')->is_ssl()) ? str_replace("http://", "https://", $this->image_url) : $this->image_url;
			$this->image_filename = basename($this->image_url);
		}

		$this->post_data = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByFlickr_post', $this->post_data, $slider_id, $this);
	...
	private function init_by_youtube($slider_id, $additions){
		$this->post_data = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByYoutube_pre', $this->post_data, $slider_id, $additions, $this);
		...
		if(in_array($bg_type, array('trans', 'image', 'streamyoutube', 'streamyoutubeboth', 'youtube', 'streamvimeo', 'streamvimeoboth', 'vimeo'), true)){ //if image is choosen, use featured image as background
			...
			$this->image_url = (empty($this->image_url)) ? Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/youtube.png' : $this->image_url;
			$this->image_url = (Mage::helper('nwdrevslider/framework')->is_ssl()) ? str_replace('http://', 'https://', $this->image_url) : $this->image_url;
		...
		$this->post_data = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByYoutube_post', $this->post_data, $slider_id, $additions, $this);
	...
	private function init_by_vimeo($slider_id, $additions){
		$this->post_data = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByVimeo_pre', $this->post_data, $slider_id, $additions, $this);
		...
		if($this->get_val($this->params, array('bg', 'imageFromStream'), false) === true && in_array($this->get_val($this->params, array('bg', 'type')), array('trans', 'image', 'streamvimeo', 'streamvimeoboth', 'vimeo'), true)){ //if image is choosen, use featured image as background
			...
			$this->image_url = $this->get_val($is, $img_res, '');
			$this->image_url = (empty($this->image_url)) ? Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/vimeo.png' : $this->image_url;
			$this->image_url = (Mage::helper('nwdrevslider/framework')->is_ssl()) ? str_replace("http://", "https://", $this->image_url) : $this->image_url;
			$this->image_thumb = $this->get_val($is, 'thumbnail', $this->image_thumb);
			$this->image_filename = basename($this->image_url);
		}

		$this->post_data = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByVimeo_post', $this->post_data, $slider_id, $additions, $this);
	...
	private function set_layers_by_stream($slider_id, $stream_type, $additions = array()){
		$a				= Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_setLayersByStreamData_pre', array('layers' => $this->layers, 'params' => $this->params), $slider_id, $stream_type, $additions, $this);
		...
		if(!empty($this->layers)){
			foreach($this->layers as $key => $layer){
				$text = $this->get_val($layer, 'text');
				$text = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_mod_stream_meta', $text, $slider_id, $stream_type, $this->post_data); //option to add your own filter here to modify meta to your likings
		...
		$a = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_setLayersByStreamData_post', array('layers' => $this->layers, 'params' => $this->params), $slider_id, $stream_type, $additions, $this);
		...
		switch($stream_type){
			...
			case 'instagram':
				...
				$attr1 = array(
					...
					'date' => Mage::helper('nwdrevslider/framework')->date_i18n(Mage::helper('nwdrevslider/framework')->get_option('date_format').' '.Mage::helper('nwdrevslider/framework')->get_option('time_format'), $this->get_val($this->post_data, 'taken_at_timestamp', false)),
	...
	public function save_layers(){
		...
		$this->layers	= Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_saveLayers', $this->layers, $this->static_slide, $this);
	...
	public function save_settings(){
		...
		$table = ($this->static_slide) ? $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES : $wpdb->prefix . RevSliderFront::TABLE_SLIDES;
		$this->settings = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_save_settings', $this->settings, $this->static_slide, $this);
	...
	public function get_children(){
		...
		return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_getArrChildren', $this->children, $this);
	}
	...
	public function create_slide($slider_id, $obj = '', $static = false, $id = false){
		...
		$insert	= Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_createSlide', $insert, $slider_id, $static, $this);
	...
	public function init_by_static_id($slide_id){
		...
		$slide = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES." WHERE id = %d", $slide_id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	public function init_by_data($slide){
		$slide = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_init_by_data', Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_initByData', $slide));
		...
		$image_url			= (Mage::helper('nwdrevslider/framework')->is_ssl()) ? str_replace('http://', 'https://', $image_url) : $image_url;
	...
	public function get_slides_for_library($slider_ids = array(), $get_static_slide = false){
		...
		if(!empty($slider_ids)){
			foreach($slider_ids as $sid){
				$cur_slides = $wpdb->get_results($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %s", $sid), Nwdthemes_Revslider_Helper_Query::ARRAY_A);

				if($get_static_slide === true){
					$static_slide = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES ." WHERE slider_id = %s", $sid), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
		...
		if(!empty($slides)){
			...
			foreach($slides as $key => $slide){
				...
				if($this->get_val($slide, 'static', false) === true){
					...
					$slides[$key]['bg'] = array('type' => 'image', 'src' => Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'admin/assets/images/sources/static.png', 'style' => array());
				}else{
	...
	public function get_slides_by_slider_id($slider_id, $published = false, $wmpl = false, $first = false, $init_layer = true){
		...
		$slides		 = array();
		$children	 = array();
		$slides_data = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_SLIDES." WHERE slider_id = %d ORDER BY slide_order ASC", $slider_id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
			...
			*/
			...
			if ($published) {
				...
				if ($storeId) {
					...
					$storeIds = [];
					foreach ($slide->get_param(array('allow_stores'), array()) as $key => $value) {
						if ($value) {
							$storeIds[] = str_replace('store', '', $key);
						}
					}
				}
				if ($storePermissions && ! in_array(Mage::app()->getStore()->getStoreId(), $storeIds)) {
					continue;
				}
			}
	...
	public function get_params_for_export(){
		...
		return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_getParamsForExport', Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_getParamsForExport', $params, $this), $this);
	}
	...
	public function get_layers_for_export(){
		...
		return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_get_layers_for_export', Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_getLayersForExport', $layers, $this), $this);
	}
		...
		if($thumb_src === false){
			if($bgtype == 'trans'){
				...
				if(!empty($bg)){
					...
					$_src = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'public/assets/assets/sources/';
					switch($layouttype){
						case 'gallery':
						break;
						case 'posts':
	...
	private function set_image_by_image_id($id){
		$id			= Mage::helper('nwdrevslider/framework')->apply_filters('revslider_slide_setImageByImageID', $id, $this);
	...
	public function get_categories_html($cat_ids, $tax = null, $post_id = ''){
		...
		if(!empty($errors)){
			...
			$this->throw_error(__('retrieving categories error: '.Mage::helper('nwdrevslider/framework')->esc_html($err)));
		}
		...
		foreach($categories as $category){
			...
			$link = Mage::helper('nwdrevslider/framework')->get_category_link($category['term_id']);
			$name = $category['name'];
			$list.= ($sep == true) ? ',' : '';
			$list.= (!empty($link)) ? '<a href="' . Mage::helper('nwdrevslider/framework')->esc_url($link) . '" title="' . Mage::helper('nwdrevslider/framework')->esc_attr(sprintf(__('View all posts in %s', 'revslider'), $category['name'])) .'" '. $rel .'>'. $name .'</a>' : $name;
			$sep  = true;
		}
	...
	public function get_content_path(){
        $path = Nwdthemes_Revslider_Helper_Framework::WP_CONTENT_DIR . '/';
		return $path;
	}
