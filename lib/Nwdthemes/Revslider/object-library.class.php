...
class RevSliderObjectLibrary extends RevSliderFunctions {
    ...
    public function __construct() {
        ...
        $this->font_icon_paths = array(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/font-awesome/css/font-awesome.css', Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'public/assets/fonts/pe-icon-7-stroke/css/pe-icon-7-stroke.css');
        $this->font_icon_paths = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_object_library_icon_paths', $this->font_icon_paths);
    }
    ...
    public function _get_list($force = false){
        ...
        if(time() - $last_check > 1296000 || $force == true){ //30 days
            ...
            $hash = Mage::helper('nwdrevslider/framework')->get_option('revslider-library-hash', '');
            ...
            $request = $this->_rslb->call_url($this->library_list, $rattr, 'library');
            ...
            if(!Mage::helper('nwdrevslider/framework')->is_wp_error($request)) {
                if($response = Mage::helper('nwdrevslider/framework')->maybe_unserialize($request['body'])) {
                    if('actual' != $response){
                        ...
                        if(is_array($library)) {
                            if(isset($library['hash'])) Mage::helper('nwdrevslider/framework')->update_option('revslider-library-hash', $library['hash']);
    ...
    public function get_object_handle_by_id($id){
        $handle     = '';
        $full     = Mage::helper('nwdrevslider/framework')->get_option('rs-library', array());
    ...
    public function _get_object_thumb($object_handle, $type, $download = false){
        ...
        if($_download && Mage::helper('nwdrevslider/filesystem')->wp_mkdir_p($upload_dir['basedir'].$path) && $download === true){
            ...
            if(!is_file($file)){
                ...
                if($curl !== false){
                    if($validated == 'false' && !in_array($type, $this->allowed_types, true)){
                        ...
                        $image_data = $this->_rslb->call_url($this->library_download, $rattr, 'library', $http_force);
    ...
    public function _get_object_layers($object_id){
        ...
        if($curl !== false){
            ...
            if($validated == 'false' && !in_array($type, $this->allowed_types)){
                ...
                $layers_data = $this->_rslb->call_url($this->library_download, $rattr, 'library');

                if(!Mage::helper('nwdrevslider/framework')->is_wp_error($layers_data) && isset($layers_data['body']) && isset($layers_data['response']) && isset($layers_data['response']['code']) && $layers_data['response']['code'] == '200'){
        ...
        if(file_exists($image_path) && $imgsize = getimagesize($image_path)) {
            ...
            foreach($this->sizes as $size){
                ...
                if( ! file_exists($targetPath) || $force) {
                    ...
                    Mage::helper('nwdrevslider/images')->image_resize_to($image_path, $width, $height, $targetPath);
                }
            }
	...
	*/
    ...
    public function get_svg_sets_url(){
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_svg_sets', $svg_sets);
    }
    ...
    public function get_svg_sets_full(){
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_svg_sets_full', $svg);
    }
    ...
    public function get_font_icons(){
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_font_icons', $font_icons);
    }
    ...
    public function get_font_tags(){
        ...
        return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_font_tags', $tags);
    }
