...
class RevSliderAdmin extends RevSliderFunctions {
    ...
    public function __construct(){
        ...
        if(!file_exists(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'admin/assets/js/plugins/utils.min.js') && !file_exists(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'admin/assets/js/modules/editor.min.js')){
            $this->dev_mode = true;
        }

        $this->path_views = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH . 'admin/views/';
    ...
    public function enqueue_admin_styles(){
        ...
        Mage::helper('nwdrevslider/framework')->wp_enqueue_style('revbuilder-color-picker-css', Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL . 'admin/assets/css/tp-color-picker.css', array(), Nwdthemes_Revslider_Helper_Framework::RS_REVISION);

        if(in_array($this->get_val(Nwdthemes_Revslider_Helper_Data::$_GET, 'page'), $this->pages)){
            Mage::helper('nwdrevslider/framework')->wp_enqueue_style('revbuilder-select2RS', Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL . 'admin/assets/css/select2RS.css', array(), Nwdthemes_Revslider_Helper_Framework::RS_REVISION);
            Mage::helper('nwdrevslider/framework')->wp_enqueue_style('rs-frontend-settings', Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL . 'public/assets/css/rs6.css', array(), Nwdthemes_Revslider_Helper_Framework::RS_REVISION);
            Mage::helper('nwdrevslider/framework')->wp_enqueue_style('rs-icon-set-fa-icon-', Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL . 'public/assets/fonts/font-awesome/css/font-awesome.css', array(), Nwdthemes_Revslider_Helper_Framework::RS_REVISION);
            Mage::helper('nwdrevslider/framework')->wp_enqueue_style('rs-icon-set-pe-7s-', Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL . 'public/assets/fonts/pe-icon-7-stroke/css/pe-icon-7-stroke.css', array(), Nwdthemes_Revslider_Helper_Framework::RS_REVISION);
            Mage::helper('nwdrevslider/framework')->wp_enqueue_style('revslider-basics-css', Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL . 'admin/assets/css/basics.css', array(), Nwdthemes_Revslider_Helper_Framework::RS_REVISION); //'rs-new-plugin-settings'
            Mage::helper('nwdrevslider/framework')->wp_enqueue_style('rs-new-plugin-settings', Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL . 'admin/assets/css/builder.css', array('revslider-basics-css'), Nwdthemes_Revslider_Helper_Framework::RS_REVISION);
            if(Mage::helper('nwdrevslider/framework')->is_rtl()){
                Mage::helper('nwdrevslider/framework')->wp_enqueue_style('rs-new-plugin-settings-rtl', Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL . 'admin/assets/css/builder-rtl.css', array('rs-new-plugin-settings'), Nwdthemes_Revslider_Helper_Framework::RS_REVISION);
            }
        }
    }
    ...
    public function enqueue_admin_scripts(){
        ...
        Mage::helper('nwdrevslider/framework')->wp_enqueue_script(array('jquery', 'jquery-ui-core', 'jquery-ui-mouse', 'jquery-ui-accordion', 'jquery-ui-datepicker', 'jquery-ui-dialog', 'jquery-ui-slider', 'jquery-ui-autocomplete', 'jquery-ui-sortable', 'jquery-ui-droppable', 'jquery-ui-tabs', 'jquery-ui-widget', 'wp-color-picker', 'wpdialogs', 'updates'));
        Mage::helper('nwdrevslider/framework')->wp_enqueue_script(array('wp-color-picker'));

        if(in_array($this->get_val(Nwdthemes_Revslider_Helper_Data::$_GET, 'page'), $this->pages)){

            $view = $this->get_val(Nwdthemes_Revslider_Helper_Data::$_GET, 'view');

            Mage::helper('nwdrevslider/framework')->wp_localize_script('revbuilder-admin', 'RVS_LANG', $this->get_javascript_multilanguage()); //Load multilanguage for JavaScript
        }
    ...
    public function add_admin_pages(){
        ...
        $this->screens[] = Mage::helper('nwdrevslider/framework')->add_menu_page('Slider Revolution', 'Slider Revolution', $role, 'revslider', array($this, 'display_admin_page'), 'dashicons-update');
    }
    ...
    private function do_update_checks(){
        $upgrade    = new RevSliderUpdate(Nwdthemes_Revslider_Helper_Framework::RS_REVISION);
        ...
        if($validated === 'true' || version_compare(Nwdthemes_Revslider_Helper_Framework::RS_REVISION, $stablev, '<')){
    ...
    function modify_admin_body_class($classes){
        if($this->get_val(Nwdthemes_Revslider_Helper_Data::$_GET, 'page') == 'revslider' && $this->get_val(Nwdthemes_Revslider_Helper_Data::$_GET, 'view') == 'slide'){
    ...
    public function add_actions(){
        ...
        Mage::helper('nwdrevslider/framework')->add_action('admin_head', array($this, 'hide_notices'), 1);
        Mage::helper('nwdrevslider/framework')->add_action('admin_menu', array($this, 'add_admin_pages'));
        Mage::helper('nwdrevslider/framework')->add_action('add_meta_boxes', array($this, 'add_slider_meta_box'));
        Mage::helper('nwdrevslider/framework')->add_action('save_post', array($this, 'on_save_post'));
        Mage::helper('nwdrevslider/framework')->add_action('admin_enqueue_scripts', array($this, 'enqueue_admin_styles'));
        Mage::helper('nwdrevslider/framework')->add_action('admin_enqueue_scripts', array($this, 'enqueue_admin_scripts'));
        Mage::helper('nwdrevslider/framework')->add_action('wp_ajax_revslider_ajax_action', array($this, 'do_ajax_action')); //ajax response to save slider options.
        Mage::helper('nwdrevslider/framework')->add_action('wp_ajax_revslider_ajax_call_front', array($this, 'do_front_ajax_action'));
        Mage::helper('nwdrevslider/framework')->add_action('wp_ajax_nopriv_revslider_ajax_call_front', array($this, 'do_front_ajax_action')); //for not logged in users

        if(isset($pagenow) && $pagenow == 'plugins.php'){
            Mage::helper('nwdrevslider/framework')->add_action('admin_notices', array($this, 'add_plugins_page_notices'));
        }

        Mage::helper('nwdrevslider/framework')->add_action('admin_init', array($this, 'add_suggested_privacy_content'), 15);
    }
    ...
    public function add_filters(){
        Mage::helper('nwdrevslider/framework')->add_filter('admin_body_class', array($this, 'modify_admin_body_class'));
    }
    ...
    public static function add_plugins_page_notices(){
        ...
        foreach($plugins as $plugin_id => $plugin){
            ...
            $add = (Mage::helper('nwdrevslider/framework')->get_option('revslider-valid', 'false') == 'false' || version_compare(Mage::helper('nwdrevslider/framework')->get_option('revslider-latest-version', Nwdthemes_Revslider_Helper_Framework::RS_REVISION), $plugin['Version'], '>')) ? true : false;

            if($add){
                Mage::helper('nwdrevslider/framework')->add_action('after_plugin_row_' . $plugin_id, array('RevSliderAdmin', 'add_notice_wrap_pre'), 10, 3);
            }

            //check version, latest updates and if registered or not
            if(Mage::helper('nwdrevslider/framework')->get_option('revslider-valid', 'false') == 'false'){
            ...
            if($add){
                Mage::helper('nwdrevslider/framework')->add_action('after_plugin_row_' . $plugin_id, array('RevSliderAdmin', 'add_notice_wrap_post'), 10, 3);
            }
        }
    }
    ...
    public static function add_notice_wrap_pre($plugin_file, $plugin_data, $plugin_status){
        ...
        if(is_network_admin()){
            ...
            $active_class = Mage::helper('nwdrevslider/framework')->is_plugin_active($plugin_file) ? ' active' : '';
        }
    ...
    public function do_ajax_action(){
        @ini_set('memory_limit', Mage::helper('nwdrevslider/framework')->apply_filters('admin_memory_limit', Nwdthemes_Revslider_Helper_Framework::WP_MAX_MEMORY_LIMIT));
        ...
        try{
            ...
            if(!Mage::helper('nwdrevslider/framework')->current_user_can('administrator') && Mage::helper('nwdrevslider/framework')->apply_filters('revslider_restrict_role', true)){
                switch($action){
            ...
            if(Mage::helper('nwdrevslider/framework')->wp_verify_nonce($nonce, 'revslider_actions') == false){
            ...
            switch($action){
                ...
                case 'dismiss_dynamic_notice':
                    ...
                    if(!empty($ids)){
                        foreach($ids as $_id){
                            $notices_discarded[] = Mage::helper('nwdrevslider/framework')->esc_attr(trim($_id));
                        }
                ...
                case 'check_for_updates':
                    $update = new RevSliderUpdate(Nwdthemes_Revslider_Helper_Framework::RS_REVISION);
                    $update->force = true;

                    $update->_retrieve_version_info();
                    $version = Mage::helper('nwdrevslider/framework')->get_option('revslider-latest-version', Nwdthemes_Revslider_Helper_Framework::RS_REVISION);
				...
				case 'get_list_of':
					...
					switch($type){
						...
						case 'posttypes':
							...
							$output = 'objects';
							$operator = 'and';
							$post_types = Mage::helper('nwdrevslider/framework')->get_post_types($args, $output, $operator);
							...
							foreach($post_types as $post_type){
								...
								if(!in_array($post_type->name, array('post', 'page', 'attachment', 'revision', 'nav_menu_item', 'custom_css', 'custom_changeset', 'user_request'))){
									$taxonomy_objects = Mage::helper('nwdrevslider/framework')->get_object_taxonomies($post_type->name, 'objects');
				...
				case 'load_builder':
					ob_start();
					require_once Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH . 'admin/views/builder.php';
				...
				case 'preview_slider':
					...
					if(intval($slider_id) > 0 && empty($slider_data)){
						...
						if(version_compare($slider->get_param(array('settings', 'version')), Mage::helper('nwdrevslider/framework')->get_option('revslider_update_revision', '6.0.0'), '<')){
						...
						$_slider = array(
							...
							'title'		=> 'Preview',
							'alias'		=> 'preview',
							'settings'	=> json_encode(array('version' => Nwdthemes_Revslider_Helper_Framework::RS_REVISION)),
						...
						foreach($slider_data as $sk => $sd){
							...
							if(strpos($sk, 'static_') !== false){
								...
								$_slides[$sortOrder] = array(
									...
									'params'	=> stripslashes($this->get_val($sd, 'params')),
									'layers'	=> stripslashes($this->get_val($sd, 'layers')),
									'settings'	=> array('version' => Nwdthemes_Revslider_Helper_Framework::RS_REVISION)
								);
							}
							$order++;
						}
                    ...
                    $html = Mage::app()
                        ...
                        ->setTemplate('nwdthemes/revslider/public/revslider-page-template.phtml')
				...
				case 'set_favorite':
					...
					$id = Mage::helper('nwdrevslider/framework')->esc_attr($this->get_val($data, 'id'));
				...
				case 'activate_addon':
					...
					if($return === true){
						...
						$data = array();
						$data = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_activate_addon', $data, $handle);
				...
				case 'create_draft_page':
					...
					if ($page_id) {
						$response['open'] = Mage::helper('cms/page')->getPageUrl($page_id);
						$response['edit'] = Mage::helper('adminhtml')->getUrl('adminhtml/cms_page/edit', array('page_id' => $page_id));
					}
				...
				break;
				case 'set_tooltip_preference':
					Mage::helper('nwdrevslider/framework')->update_option('revslider_hide_tooltips', true);
				...
				break;
				case 'fix_database_issues':
					Mage::helper('nwdrevslider/framework')->update_option('revslider_table_version', '1.0.0');
				...
				break;
				default:
					$return = ''; //''is not allowed to be added directly in Mage::helper('nwdrevslider/framework')->apply_filters(), so its needed like this
					$return = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_do_ajax', $return, $action, $data);
					if($return){
						if(is_array($return)){
			...
			if(in_array($action, array('preview_slide', 'preview_slider'))){
				echo $message;
				Mage::helper('nwdrevslider/framework')->wp_die();
			}
	...
	public function do_front_ajax_action(){
		...
		//verify the token
		$is_verified = Mage::helper('nwdrevslider/framework')->wp_verify_nonce($token, 'RevSlider_Front');
	...
	public function get_javascript_multilanguage(){
		...
		return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_get_javascript_multilanguage', $lang);
	}
	...
	public function show_error($view, $message){
		...
		echo Mage::helper('nwdrevslider/framework')->esc_attr($view);
		echo ' - Error: <span>';
		echo Mage::helper('nwdrevslider/framework')->esc_attr($message);
		echo '</span>';
		echo '</div>';
		exit;
	}
	...
	public function validate_filepath($filepath, $prefix = null){
		...
		$message = $prefix.' '.Mage::helper('nwdrevslider/framework')->esc_attr($filepath).' not exists!';
	...
	public function activate_plugin($code){
		...
		if(Mage::helper('nwdrevslider/framework')->is_wp_error($version_info)){
			return false;
		}

		if($version_info == 'valid'){
			Mage::helper('nwdrevslider/framework')->update_option('revslider-valid', 'true');
			Mage::helper('nwdrevslider/framework')->update_option('revslider-code', $code);
			return true;
	...
	public function deactivate_plugin(){
		...
		$vi	 = Mage::helper('nwdrevslider/framework')->wp_remote_retrieve_body($res);

		if(Mage::helper('nwdrevslider/framework')->is_wp_error($vi)){
			return false;
		}
	...
	public function create_fake_post($content, $title = 'Slider Revolution'){
		...
		$post->post_date	 = Mage::helper('nwdrevslider/framework')->current_time('mysql');
		$post->post_date_gmt = Mage::helper('nwdrevslider/framework')->current_time('mysql', 1);
	...
	public static function esc_js_deep($value){
		$value = is_array($value) ? array_map(array('RevSliderAdmin', 'esc_js_deep'), $value) : Mage::helper('nwdrevslider/framework')->esc_js($value);
	...
	public function generate_attachment_metadata(){
		$rs_meta_create = Mage::helper('nwdrevslider/framework')->get_option('rs_image_meta_todo', array());
		if(!empty($rs_meta_create)){
			foreach($rs_meta_create as $attach_id => $save_dir){
				if($attach_data = Mage::helper('nwdrevslider/framework')->wp_generate_attachment_metadata($attach_id, $save_dir)){
					Mage::helper('nwdrevslider/framework')->wp_update_attachment_metadata($attach_id, $attach_data);
				}
				unset($rs_meta_create[$attach_id]);

				Mage::helper('nwdrevslider/framework')->update_option('rs_image_meta_todo', $rs_meta_create);
			}
		}
	}
