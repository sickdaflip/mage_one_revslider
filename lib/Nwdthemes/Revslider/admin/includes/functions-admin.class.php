...
class RevSliderFunctionsAdmin extends RevSliderFunctions {
	...
	public function add_notices(){
		...
		/*if(!Mage::helper('nwdrevslider/framework')->current_user_can('administrator')){
			return $_n;
		}*/
		...
		if(!empty($notices) && is_array($notices)){
			$n_discarted = Mage::helper('nwdrevslider/framework')->get_option('revslider-notices-dc', array());

			foreach($notices as $notice){
				//check if global or just on plugin related pages
				if($notice->version === true || !in_array($notice->code, $n_discarted) && version_compare($notice->version, Nwdthemes_Revslider_Helper_Framework::RS_REVISION, '>=')){
					$_n[] = $notice;
				}
			}
		}
	...
	public function get_v5_slider_data(){
		...
		$slider_data = $wpdb->get_results($wpdb->prepare("SELECT `id`, `title`, `alias`, `type` FROM ".$wpdb->prefix . RevSliderFront::TABLE_SLIDER."_bkp ORDER BY %s %s", array($do_order, $direction)), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	public function reimport_v5_slider($id){
		...
		$slider_data = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_SLIDER."_bkp WHERE `id` = %s", $id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);

		if(!empty($slider_data)){
			$slides_data = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_SLIDES."_bkp WHERE `slider_id` = %s", $id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
			$static_slide_data = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES."_bkp WHERE `slider_id` = %s", $id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);

			if(!empty($slides_data)){
				//check if the ID's exist in the new tables, if yes overwrite, if not create
				$slider_v6 = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_SLIDER." WHERE `id` = %s", $id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
				...
				if($id !== false){
					foreach($slides_data as $k => $slide_data){
						...
						$slide_v6 = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_SLIDES." WHERE `id` = %s", $slide_data['id']), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
					...
					if(!empty($static_slide_data)){
						...
						$slide_v6 = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES." WHERE `id` = %s", $static_slide_data['id']), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
