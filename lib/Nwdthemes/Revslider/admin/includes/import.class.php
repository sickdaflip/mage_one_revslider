...
class RevSliderSliderImport extends RevSliderSlider {
	...
	public function import_slider($update_animation = true, $exact_filepath = false, $is_template = false, $single_slide = false, $update_navigation = true, $install = true){
		...
		Mage::helper('nwdrevslider/framework')->do_action('revslider_slider_imported', $this->real_slider_id);
	...
	private function unzip_slider($exact_filepath = false){
		...
		$file = Mage::helper('nwdrevslider/filesystem')->unzip_file($path, $this->download_path);

		if(Mage::helper('nwdrevslider/framework')->is_wp_error($file)){
			@define('FS_METHOD', 'direct'); //lets try direct.

			$file = Mage::helper('nwdrevslider/filesystem')->unzip_file($path, $this->download_path);
			if(Mage::helper('nwdrevslider/framework')->is_wp_error($file)){
				$this->download_path = Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'rstemp/';
		...
		if(!Mage::helper('nwdrevslider/framework')->is_wp_error($this->unzipped_data)){
	...
	public function set_animations($update){
		...
		if(!empty($animations)){
			foreach($animations as $animation){
				$exist = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_LAYER_ANIMATIONS." WHERE handle = %s", $animation['handle']), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	public function set_dynamic_css_v5(){
		...
		if(is_array($css) && $css !== false && count($css) > 0){
			foreach($css as $class => $styles){
				...
				$result = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_CSS." WHERE handle = %s", $class), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	public function set_dynamic_css_v6(){
		...
		if(!empty($styles)){
			foreach($styles as $style){
				...
				$exist = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".$wpdb->prefix . RevSliderFront::TABLE_CSS." WHERE handle = %s", $this->get_val($style, 'handle')), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
				if(!empty($exist)){
			...
			foreach($navigations as $navigation){
				...
				if(!empty($_navigations)){
					foreach($_navigations as $navigation){
						$exist = $wpdb->get_row($wpdb->prepare("SELECT id FROM ".$wpdb->prefix . RevSliderFront::TABLE_NAVIGATIONS." WHERE handle = %s AND type = %s", array($this->get_val($navigation, 'handle'), $this->get_val($navigation, 'type'))), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	public function process_slide_data_pre_6(){
		...
		foreach($this->slides_data as $slide_key => $slide){
					...
					if($this->import_zip === true){ //we have a zip, check if exists
						...
						if(isset($layer['type']) && $layer['type'] == 'svg'){
							if(isset($layer['svg']) && isset($layer['svg']->src)){
								$layer['svg']->src = Mage::helper('nwdrevslider/framework')->content_url().$layer['svg']->src;
							}
						}
					}
			...
			$d = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_importSliderFromPost_modify_data', $d, 'normal', $this->download_path);
				...
				foreach($layers as $layer_key => $layer){
					...
					if($this->import_zip === true){ //we have a zip, check if exists
						...
						if($layer_type == 'svg'){
							...
							if(!empty($svg)) $layer['svg']['source'] = Mage::helper('nwdrevslider/framework')->content_url().$svg;
						}
					}
	...
	public function process_static_slide_data_pre_6(){
		...
		if(!empty($static_slide) && $this->import_statics){
			foreach($static_slide as $slide){
				...
				$d = Mage::helper('nwdrevslider/framework')->apply_filters('revslider_importSliderFromPost_modify_data', $d, 'static', $this->download_path);
					...
					foreach($layers as $layer_key => $layer){
						...
						if(isset($layer['type']) && $layer['type'] == 'svg'){
							if(isset($layer['svgSource'])){
								$layer['svgSource'] = Mage::helper('nwdrevslider/framework')->content_url().$layer['svgSource'];
							}
						}
	...
	public function import_media_from_zip($image){
		...
		if($image !== '' && strpos($image, 'http') === false){
			if($this->import_zip === true){ //we have a zip, check if exists
				...
				if(!$exists){
					//echo '<p>'.Mage::helper('nwdrevslider/framework')->esc_attr($image).__(' not found!', 'revslider').'</p>';
				}else{
