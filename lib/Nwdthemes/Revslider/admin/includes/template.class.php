...
class RevSliderTemplate extends RevSliderFunctions {
	...
	public function _get_template_list($force = false){
		...
		if(time() - $last_check > 345600 || $force == true){ //4 days
			...
			$hash = Mage::helper('nwdrevslider/framework')->get_option('revslider-templates-hash', '');
			$code	= (Mage::helper('nwdrevslider/framework')->get_option('revslider-valid', 'false') == 'false') ? '' : Mage::helper('nwdrevslider/framework')->get_option('revslider-code', '');
			$data	= array(
				'code'		=> urlencode($code),
				'shop_version' => urlencode(self::SHOP_VERSION),
			...
			if(!Mage::helper('nwdrevslider/framework')->is_wp_error($request)){
				if($response = Mage::helper('nwdrevslider/framework')->maybe_unserialize($request['body'])){
					...
					if(is_array($templates)){
						if(isset($templates['hash'])) Mage::helper('nwdrevslider/framework')->update_option('revslider-templates-hash', $templates['hash']);
						Mage::helper('nwdrevslider/framework')->update_option('rs-templates-new', $templates, false);
					}
				}
			}
	...
	private function update_template_list(){
		...
		if($new !== false && !empty($new) && is_array($new)){
			...
			Mage::helper('nwdrevslider/framework')->update_option('rs-templates', $cur, false);
			Mage::helper('nwdrevslider/framework')->update_option('rs-templates-new', false, false);
	...
	private function _update_images($img = false){
						...
						foreach($temp as $k => $tvalues){
							...
							if(Mage::helper('nwdrevslider/filesystem')->wp_mkdir_p($upload_dir['basedir']. $this->templates_path)){
								...
								if(!file_exists($file) || isset($reload[$key])){ //update, so load again
									if($curl !== false){
										//curl_setopt( $curl, CURLOPT_CAINFO, Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH.'cert.crt'); //'sslcertificates'
										$done	= false;
										$count	= 0;
										do{
		...
		Mage::helper('nwdrevslider/framework')->update_option('rs-templates', $templates, false); //remove the push_image
	}
	...
	public function copy_slide_to_templates($slide_id, $slide_title, $slide_settings = array()){
		...
		$duplicate = $wpdb->get_row($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE id = %s", $slide_id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	public function get_template_slides(){
		...
		$templates	= $wpdb->get_results($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %s", -1), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
		...
		if(!empty($sliders)){
			foreach($sliders as $slider){
				...
				if($this->get_val($slider, 'installed', false) !== false){
					$cur_slides = $wpdb->get_results($wpdb->prepare("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDES ." WHERE slider_id = %s", $slider['installed']), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
					$installed	= true;
				}else{
					$cur_slides = $slides;
				}
	...
	public function get_default_template_sliders(){
		...
		$check = $wpdb->get_results("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE type = 'template'", Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	public function get_tp_template_sliders($uid = false){
		...
		//add themepunch default Sliders here
		$sliders = $wpdb->get_results("SELECT * FROM ". $wpdb->prefix . RevSliderFront::TABLE_SLIDER ." WHERE type = 'template'", Nwdthemes_Revslider_Helper_Query::ARRAY_A);
				...
				if(!empty($defaults[$dk]['plugin_require'])){
					foreach($defaults[$dk]['plugin_require'] as $pr => $plugin){
						...
						if(!isset($plugin_list[$path])){
							$plugin_list[$path] = (Mage::helper('nwdrevslider/framework')->is_plugin_active(Mage::helper('nwdrevslider/framework')->esc_attr($path))) ? true : false;
						}
