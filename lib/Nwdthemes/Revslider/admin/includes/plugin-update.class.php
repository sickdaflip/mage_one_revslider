...
class RevSliderPluginUpdate extends RevSliderFunctions {
	...
	public function __construct(){
		$this->revision = Mage::helper('nwdrevslider/framework')->get_option('revslider_update_revision', '6.0.0');
	...
	public function slider_need_update_checks(){
		$finished = Mage::helper('nwdrevslider/framework')->get_option('revslider_update_revision_current', '1.0.0');
	...
	public function upgrade_next_slider(){
		...
		Mage::helper('nwdrevslider/framework')->update_option('revslider_update_revision_current', $this->revision);
	...
	public function add_v5_styles(){
		...
		foreach($v5 as $v5class){
			$result = $wpdb->get_row($wpdb->prepare("SELECT id FROM " . $wpdb->prefix . RevSliderFront::TABLE_CSS . " WHERE handle = %s", $v5class['handle']), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
			if(empty($result)){
				//add v5 style
	...
	public function update_css_styles(){
		...
		$styles = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . RevSliderFront::TABLE_CSS, Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	public function check_settings_table(){
		...
		if($wpdb->get_var("SHOW TABLES LIKE '" . $wpdb->prefix . RevSliderFront::TABLE_SETTINGS . "'") == $wpdb->prefix . RevSliderFront::TABLE_SETTINGS){
			$result = $wpdb->get_row("SELECT `general` FROM " . $wpdb->prefix . RevSliderFront::TABLE_SETTINGS, Nwdthemes_Revslider_Helper_Query::ARRAY_A);
				...
				if($template_id > 0){
					...
					foreach($temp_sliders as $t_slider){
						if($t_slider->get_id() === $template_id){
							...
							if($static_id !== false){
								$record = $wpdb->get_row($wpdb->prepare("SELECT * FROM " . $wpdb->prefix . RevSliderFront::TABLE_STATIC_SLIDES . " WHERE id = %d", $static_id), Nwdthemes_Revslider_Helper_Query::ARRAY_A);
				...
				if(!empty($slides) && is_array($slides)){
					foreach($slides as $slide){
						...
						if($ml_id !== false && $ml_id !== $image_id){
							$urlImage = Mage::helper('nwdrevslider/images')->wp_get_attachment_image_src($ml_id, 'full');
	...
	public function migrate_layer_to_6_0($layer, $blank = false, $slide, $slider){
		...
		switch($ml['type']){
			...
			case 'button':
				...
				$ml['media'] = array(
					'imageUrl' => $this->get_val($layer, 'bgimage_url', Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'admin/assets/images/transparent_placeholder.png'),
					//'imageId' => $img_id,
			...
			case 'image':
				...
				if($img_id === false) $img_id = '';
				$ml['media'] = array(
					'imageUrl' => $this->get_val($layer, 'image_url', Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_URL.'admin/assets/images/transparent_placeholder.png'),
					'imageId' => $img_id,
					'imageFromStream' => false,
					'loaded' => false
				);
				...
				if($this->get_val($layer, 'scaleX', false) !== false){
					...
					if($get_dim === true){
						...
						if($cur_img !== ''){
							...
							$img_data = Mage::helper('nwdrevslider/framework')->wp_get_attachment_metadata($cur_id);
	...
	private function change_global_addon_settings_to_6_0(){
        ...
        // 404 pages
        if(Mage::helper('nwdrevslider/framework')->is_plugin_active('revslider-404-addon/revslider-404-addon.php')){

            $settings = Mage::helper('nwdrevslider/framework')->get_option('revslider_404_addon');
            if($settings !== false){
                $options = array();
                parse_str($settings, $options);
                if(isset($options['revslider-404-addon-active']) && intval($options['revslider-404-addon-active'])){
                    Mage::helper('nwdrevslider/framework')->update_option('revslider_404_enabled', 1);
                }
            }
        ...
        if(Mage::helper('nwdrevslider/framework')->is_plugin_active('revslider-maintenance-addon/revslider-maintenance-addon.php')){

            $settings = Mage::helper('nwdrevslider/framework')->get_option('revslider_maintenance_addon');
            if($settings !== false){
                $options = array();
                parse_str($settings, $options);
                if(isset($options['revslider-maintenance-addon-active']) && intval($options['revslider-maintenance-addon-active'])){
                    Mage::helper('nwdrevslider/framework')->update_option('revslider_maintenance_enabled', 1);
                }
            }
        ...
        if(Mage::helper('nwdrevslider/framework')->is_plugin_active('revslider-backup-addon/revslider-backup-addon.php')){
            Mage::helper('nwdrevslider/framework')->update_option('revslider_backup_enabled', 1);
        }

        // featured slider
        if(Mage::helper('nwdrevslider/framework')->is_plugin_active('revslider-featured-addon/revslider-featured-addon.php')){
            Mage::helper('nwdrevslider/framework')->update_option('revslider_featured_enabled', 1);
        }

        // wp gallery
        if(Mage::helper('nwdrevslider/framework')->is_plugin_active('revslider-gallery-addon/revslider-gallery-addon.php')){
            Mage::helper('nwdrevslider/framework')->update_option('revslider_gallery_enabled', 1);
        }

        // login
        if(Mage::helper('nwdrevslider/framework')->is_plugin_active('revslider-login-addon/revslider-login-addon.php')){
            Mage::helper('nwdrevslider/framework')->update_option('revslider_login_enabled', 1);
        }

        // social sharing
        if(Mage::helper('nwdrevslider/framework')->is_plugin_active('revslider-sharing-addon/revslider-sharing-addon')){
            Mage::helper('nwdrevslider/framework')->update_option('revslider_sharing_enabled', 1);
        }

        // related posts slider
        if(Mage::helper('nwdrevslider/framework')->is_plugin_active('revslider-rel-posts-addon/revslider-rel-posts-addon.php')){
            Mage::helper('nwdrevslider/framework')->update_option('revslider_rel_posts_enabled', 1);
        }

        // prev/next slider
        if(Mage::helper('nwdrevslider/framework')->is_plugin_active('revslider-prevnext-posts-addon/revslider-prevnext-posts-addon.php')){
            Mage::helper('nwdrevslider/framework')->update_option('revslider_prevnext_posts_enabled', 1);
        }
	...
	public function change_global_settings_to_6_0(){
		$global = Mage::helper('nwdrevslider/framework')->maybe_unserialize(Mage::helper('nwdrevslider/framework')->get_option('revslider-global-settings', '')); //get the old structure as serialized
		...
		if($_->get_param('wb_enable', false) !== false){
			$obj['revslider-whiteboard-addon'] = array(
				...
				'writehand' => array(
					...
					'source' => ($_->get_param('wb_writehand_source') === '1') ?  Nwdthemes_Revslider_Helper_Framework::$WP_PLUGIN_URL .'/revslider-whiteboard-addon/'.'assets/images/write_right_angle.png' : $_->get_param('wb_writehand_source_custom',  Nwdthemes_Revslider_Helper_Framework::$WP_PLUGIN_URL .'/revslider-whiteboard-addon/'.'assets/images/write_right_angle.png'),
				),
				'movehand' => array(
					...
					'source' => ($_->get_param('wb_movehand_source') === '1') ?  Nwdthemes_Revslider_Helper_Framework::$WP_PLUGIN_URL .'/revslider-whiteboard-addon/'.'assets/images/hand_point_right.png' : $_->get_param('wb_movehand_source_custom',  Nwdthemes_Revslider_Helper_Framework::$WP_PLUGIN_URL .'/revslider-whiteboard-addon/'.'assets/images/hand_point_right.png'),
				),
			);
		}
