...
class RevSliderFacebook extends RevSliderFunctions {
	...
	*/
	...
	public function get_photo_set_photos($photo_set_id, $item_count = 10, $app_id, $app_secret){
		...
		if($data !== ''){
			Mage::helper('nwdrevslider/framework')->set_transient($transient_name, $data, $this->transient_sec);
		}
	...
	public function get_photo_set_photos_options($user_url, $current_album, $app_id, $app_secret, $item_count = 99){
		...
		if(is_array($photo_sets)){
			foreach($photo_sets as $photo_set){
				$return[] = '<option title="'.$this->get_val($photo_set, 'name').'" '.Mage::helper('nwdrevslider/framework')->selected($this->get_val($photo_set, 'id'), $current_album, false).' value="'.$this->get_val($photo_set, 'id').'">'.$this->get_val($photo_set, 'name').'</option>"';
			}
		}
		return $return;
	}
...
class RevSliderTwitterApi extends RevSliderFunctions {
	...
	*/
	private function get_bearer_token(){
		...
		Mage::helper('nwdrevslider/framework')->update_option('twitter_bearer_token', $this->get_val($result, 'access_token'));
	...
	*/
	public function query($query, $query_args = array(), $stop = false){
		...
		if($this->get_val($this->query_args, 'cache', 0) > 0 && false !== ($data = Mage::helper('nwdrevslider/framework')->get_transient($transient_name)))
		...
		$response = Mage::helper('nwdrevslider/framework')->wp_remote_get('https://api.twitter.com/1.1/'. $this->get_val($this->query_args, 'type') . '.json?' . $query, $args);
		if(Mage::helper('nwdrevslider/framework')->is_wp_error($response) || 200 != $response['response']['code']){
			if(!$stop){
				$this->bearer_token = $this->get_bearer_token();
...
class RevSliderInstagram  extends RevSliderFunctions {
	...
	*/
	 ...
	 */
	public function __construct($transient_sec=1200){
		spl_autoload_register('rev_instagram_autoloader', true, true );
	...
	public function get_public_photos($search_user_id,$count,$orig_image=""){
		...
		if(!empty($search_user_id)){
			...
			if ($this->transient_sec > 0 && false !== ($data = Mage::helper('nwdrevslider/framework')->get_transient( $transient_name))){
				...
				Mage::helper('nwdrevslider/framework')->delete_transient( $transient_name );
				...
				if(isset($rsp->edge_owner_to_timeline_media))
					...
					if(!empty($this->stream)){
						Mage::helper('nwdrevslider/framework')->set_transient( $transient_name, $this->stream, $this->transient_sec );
						return $this->stream;
					}
					else {
	...
	private function instagram_output_array_places($photos,$count,$search_user_id,$orig_image=""){
		foreach ($photos as $photo) {
			if($count > 0){
				...
				$stream['title'] = $text;
				$stream['content'] = $text;
				$stream['date'] = Mage::helper('nwdrevslider/framework')->date_i18n( Mage::helper('nwdrevslider/framework')->get_option( 'date_format' ), ( $photo->date ) ) ;
				$stream['date_modified'] = Mage::helper('nwdrevslider/framework')->date_i18n( Mage::helper('nwdrevslider/framework')->get_option( 'date_format' ), ( $photo->date ) ) ;
