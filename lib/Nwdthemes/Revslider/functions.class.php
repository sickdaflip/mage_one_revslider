...
class RevSliderFunctions extends RevSliderData {
	...
	public function get_global_settings(){
		$gs = Mage::helper('nwdrevslider/framework')->get_option('revslider-global-settings', '');
		if(!is_array($gs)){
			$gs = json_decode($gs, true);
		}

		return Mage::helper('nwdrevslider/framework')->apply_filters('rs_get_global_settings', $gs);
	}
	...
	public function set_global_settings($global){
		...
		Mage::helper('nwdrevslider/framework')->update_option('revslider-global-settings', $global);
		return true;
	}
	...
	public function get_get_var($key, $default = ''){
		$val = $this->get_var(Nwdthemes_Revslider_Helper_Data::$_GET, $key, $default);
	...
	public function get_request_var($key, $default = ''){
		$val = (array_key_exists($key, $_POST)) ? $this->get_var($_POST, $key, $default) : $this->get_var(Nwdthemes_Revslider_Helper_Data::$_GET, $key, $default);
	...
	public function get_url_attachment_image($id, $size = 'full'){
		$image	= Mage::helper('nwdrevslider/images')->wp_get_attachment_image_src($id, $size);
		$url	= (empty($image)) ? false : $this->get_val($image, 0);
		if($url === false){
			$url = Mage::helper('nwdrevslider/framework')->wp_get_attachment_url($id);
		}
	...
	public static function get_base_url(){
		if(Mage::helper('nwdrevslider/framework')->is_multisite() == false){ //without multisite
			$url = Mage::helper('nwdrevslider/framework')->content_url().'/';
		}else{	//for multisite
	...
	public static function esc_attr_deep($value){
		$value = is_array($value) ? array_map(array('RevSliderFunctions', 'esc_attr_deep'), $value) : Mage::helper('nwdrevslider/framework')->esc_attr($value);
	...
	public function get_post_type_assoc($put_to_top = array()){
		...
		$custom_types	= Mage::helper('nwdrevslider/framework')->get_post_types(array('_builtin' => false));
	...
	public static function get_post_type_title($post_type){
		$obj_type	= Mage::helper('nwdrevslider/framework')->get_post_type_object($post_type);
	...
	public function get_post_type_taxonomies($post_type){
		$names	= array();
		$tax	= Mage::helper('nwdrevslider/framework')->get_object_taxonomies(array('post_type' => $post_type), 'objects');
	...
	public function import_media($file_url, $folder_name){
		...
		if(@fclose(@fopen($file_url, 'r'))){ //make sure the file actually exists
			...
			if($atc_id == false || $atc_id == NULL){
				...
				$attach_id = Mage::helper('nwdrevslider/framework')->wp_insert_attachment($artdata, $s_dir);

				//generate metadata and thumbnails
				Mage::helper('nwdrevslider/framework')->add_filter('intermediate_image_sizes_advanced', array('RevSliderFunctions', 'temporary_remove_sizes'), 10, 2);
	...
	public function get_static_css(){
		if(!Mage::helper('nwdrevslider/framework')->get_option('revslider-static-css')){
			if(file_exists(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH . 'public/assets/css/static-captions.css')){
				$css = @file_get_contents(Nwdthemes_Revslider_Helper_Framework::$RS_PLUGIN_PATH . 'public/assets/css/static-captions.css');
				$this->update_static_css($css);
			}
		}

		return Mage::helper('nwdrevslider/framework')->get_option('revslider-static-css', '');
	}
	...
	public function update_static_css($content){
		...
		Mage::helper('nwdrevslider/framework')->update_option('revslider-static-css', $content, 'off');
		...
		if($fdl === 'preload'){
			if(!empty($fonts)){
				...
				foreach($fonts as $key => $font){
					...
					if(!is_file($base_dir.'/gfonts/'. $font_name . '/' . $font_name . '.woff2') || filemtime($base_dir.'/gfonts/'. $font_name . '/' . $font_name . '.woff2') < $rs_google_ts){
						...
						if(preg_match_all($regex_url, $body, $found_fonts)){
							foreach($found_fonts as $found_font){
								...
								$file = $base_dir.'/gfonts/'. $font_name . '/' . $font_name . '.woff2';
								@mkdir(dirname($file));
...
"@font-face {
  ...
  font-style: normal;
  font-weight: ".$weight.";
  src: local('".$f_family."'), local('".$f_family."'), url(".$base_url.'/gfonts/'. $font_name . '/' . $font_name . '.woff2'.") format('woff2');
}";
						}
						$ret .= '</style>';
					}
	...
	public function convert_post_date($date, $with_time = false){
		if(!empty($date)){
			$date = ($with_time) ? Mage::helper('nwdrevslider/framework')->date_i18n(Mage::helper('nwdrevslider/framework')->get_option('date_format').' '.Mage::helper('nwdrevslider/framework')->get_option('time_format'), strtotime($date)) : Mage::helper('nwdrevslider/framework')->date_i18n(Mage::helper('nwdrevslider/framework')->get_option('date_format'), strtotime($date));
		}
		return $date;
	}
	...
	public function get_system_requirements(){
		$dir	= Mage::helper('nwdrevslider/framework')->wp_upload_dir();
		$basedir = $this->get_val($dir, 'basedir').'/';
		$ml		= ini_get('memory_limit');
		$mlb	= Mage::helper('nwdrevslider/framework')->wp_convert_hr_to_bytes($ml);
		$umf	= ini_get('upload_max_filesize');
		$umfb	= Mage::helper('nwdrevslider/framework')->wp_convert_hr_to_bytes($umf);
		$pms	= ini_get('post_max_size');
		$pmsb	= Mage::helper('nwdrevslider/framework')->wp_convert_hr_to_bytes($pms);
		...
		return array(
			'memory_limit' => array(
				'has' => Mage::helper('nwdrevslider/framework')->size_format($mlb),
				'min' => Mage::helper('nwdrevslider/framework')->size_format(268435456),
				'good'=> $mlg
			),
			'upload_max_filesize' => array(
				'has' => Mage::helper('nwdrevslider/framework')->size_format($umfb),
				'min' => Mage::helper('nwdrevslider/framework')->size_format(33554432),
				'good'=> $umfg
			),
			'post_max_size' => array(
				'has' => Mage::helper('nwdrevslider/framework')->size_format($pmsb),
				'min' => Mage::helper('nwdrevslider/framework')->size_format(33554432),
				'good'=> $pmsg
			),
			'upload_folder_writable'	=> Mage::helper('nwdrevslider/framework')->wp_is_writable($basedir),
			'object_library_writable'	=> true, // TODO: Remove or make it check media writable?
			'server_connect'			=> Mage::helper('nwdrevslider/framework')->get_option('revslider-connection', false),
		);
	}
