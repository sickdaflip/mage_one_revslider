...
class RevSliderData {
	...
	public function get_font_familys(){
		...
		return Mage::helper('nwdrevslider/framework')->apply_filters('revslider_data_get_font_familys', Mage::helper('nwdrevslider/framework')->apply_filters('revslider_operations_getArrFontFamilys', $fonts));
	}
	...
	public function fill_animations(){
		if(empty($this->animations)){
			...
			$result = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . RevSliderFront::TABLE_LAYER_ANIMATIONS, Nwdthemes_Revslider_Helper_Query::ARRAY_A);
	...
	public function fill_css(){
		if(empty($this->css)){
			...
			$css_data = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . RevSliderFront::TABLE_CSS, Nwdthemes_Revslider_Helper_Query::ARRAY_A);
